...
- Full suite: `venv/bin/python -m pytest` — ~161 tests, ~2s, fully mocked (see Testing & verification)
...
- `python -m pytest` runs the full suite (config in `pyproject.toml`) — ~161 tests, ~2 seconds, all
...
- [discovery-modules.md](docs/agent-notes/discovery-modules.md) — why `discoveries_manager.py`
  ...
  try to evade the latter), why `spotify_fetcher.py` scrapes the public web player's `data-testid`
  markup instead of the official (credential-requiring) Web API, and how `discovery_stats.py`
  counts per-fetcher invocations/successes for the Statistics menu.
