...
Currently 30 tests, all fully mocked — no live network calls, no scraping, no real MusicBrainz/
...
| `test_youtube_search.py` | `manage_youtube_playlists.score_result()` / `search_video_ytdlp()` — the YouTube match-ranking heuristic. 27 tests, most pinned to one specific real-world song each (a regression catalog of past wrong-match bugs — see `docs/agent-notes/youtube-search-matching.md` for the full list and the reasoning behind the heuristic). |
